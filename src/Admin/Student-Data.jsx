...
const StudentData = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const filtered = userResponse.data.filter(member => member.role_id === 1);
  ...
  const handleDelete = async (member_id) => {
    try {
      ...
      if (result.isConfirmed) {
        await axios.delete(`http://localhost:9999/auth/delete/${member_id}`);
        Swal.fire(
          'ลบแล้ว!',
        ...
        setFilteredUsers(filteredUsers.filter(member => member.member_id !== member_id));
      }
    } catch (err) {
      Swal.fire({
        icon: 'error',
  ...
  return (
    <div className="min-h-screen p-6 pl-72 bg-gray-100">
      <h1 className="text-3xl font-bold mb-8 text-center text-gray-800">ข้อมูลนักศึกษา</h1>
        ...
        <table className="min-w-full bg-white rounded-lg shadow-md">
          <thead className="bg-blue-500 text-white">
            <tr>
              ...
              <th className="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider">รหัสนักศึกษา</th>
            ...
            {paginatedData.length > 0 ? (
              paginatedData.map((member, index) => (
                <tr key={member.username_id} className="hover:bg-gray-100">
                  ...
                  <td className="px-6 py-4 text-sm text-gray-700">{member.Id_student}</td>
              ...
              <tr>
                <td colSpan="12" className="px-6 py-4 text-center text-sm text-gray-500">ไม่พบข้อมูล</td>
              </tr>
            )}
          </tbody>
        </table>
      ...
      <div className="flex justify-center mt-4">
        ...
        {[...Array(totalPages)].map((_, index) => (
          <button
            ...
            className={`px-4 py-2 mx-1 text-white rounded-lg ${currentPage === index + 1 ? 'bg-blue-700' : 'bg-blue-500 hover:bg-blue-600'}`}
          >
            {index + 1}
          </button>
        ))}
